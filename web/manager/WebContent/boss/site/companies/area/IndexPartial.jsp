...
<div class="createButton">
    <div class="float-left">
        <soul:button tag="button" target="${root}/siteConfineArea/shareHolderCreate.html?search.siteId=${command.search.siteId}"
             text="新增" title="新增" opType="dialog"
             cssClass="btn btn-info btn-addon" size="open-dialog-50" permission="blacklist:black_new_create"
    ...
    <div class="function-menu-show hide">
            <soul:button precall="confirmMessage" tag="button" text="${views.contacts['page.contacts.del']}"
                          target="${root}/siteConfineArea/shareHolderBatchDeleteArea.html" post="getSelectIds"
                         opType="ajax" cssClass="btn btn-danger-hide" callback="query" permission="blacklist:black_delete"><i
                ...
                <tr class="tab-detail">
                    ...
                    <td>
                        <c:if test="${p.builtIn ne 'true'}">
                            <soul:button target="${root}/siteConfineArea/shareHolderEdit.html?search.id=${p.id}" permission="blacklist:black_edit"  size="open-dialog-50" text="${views.common['edit']}" cssClass="co-blue m-r-xs m-l-xs" opType="dialog" callback="callBackQuery" />
                        </c:if>
                    </td>
                </tr>
            </c:forEach>
