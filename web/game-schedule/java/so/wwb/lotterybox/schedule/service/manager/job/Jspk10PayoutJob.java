 ...
 */
public class Jspk10PayoutJob extends LotterySystemJob {

    private static final Log LOG = LogFactory.getLog(Jspk10PayoutJob.class);
    ...
    public void execute(String group) {
        ...
        LOG.info("自主彩派彩任务开始code:{0},期号:{1}", JSPK10, expect);
        ...
        if(CollectionTool.isEmpty(lotteryList)){
            LOG.error("自主彩派彩前校验未通过,code:{0},expect:{1},原因{2}",JSPK10,expect,"未存在需要派彩的商户");
            return;
        }
        ...
        for(Lottery lottery : lotteryList){
            ...
            resultNumberVo.getSearch().setCode(JSPK10);
