 ...
 */
public class Jisk3OpenJob extends LotterySystemJob{

    private static final Log LOG = LogFactory.getLog(Jisk3OpenJob.class);
    ...
    public void execute(String group) {
        ...
        LOG.info("自主彩采集任务开始code:{0},期号:{1}", JISK3, expect);
        ...
        if(CollectionTool.isEmpty(lotteryList)){
            LOG.error("自主彩极速快3采集前校验未通过,code:{0},expect:{1},原因{2}",JISK3,expect,"未存在需要采集派彩的商户");
            return;
        }
