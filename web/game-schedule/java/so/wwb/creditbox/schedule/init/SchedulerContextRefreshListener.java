...
import so.wwb.creditbox.utility.ScheduleTool;
...
public class SchedulerContextRefreshListener implements ApplicationListener<ContextRefreshedEvent>,Ordered {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        try {
            if (startScheduleService(TaskSchedulerEnum.GAME.getCode())) {
                ServiceCtxLoaderListener.gameScheduleStart = true;
            }
    ...
    private boolean startScheduleService(String scheduleKey) {
        ...
        try {
            ...
            LOG.info("game-schedule:{0},初始化成功",scheduleKey);
        } catch (Exception e) {
            LOG.error(e,"game-schedule:{0},初始化异常:{1}",scheduleKey,e.getMessage());
            result = false;
        }
        return result;
    }
