 ...
 */
public class DistributorWebSocketHandler extends LbWebSocketHandler {
    private static final Log LOG = LogFactory.getLog(DistributorWebSocketHandler.class);
    ...
    public void sendMessage(MessageVo message) {
        ...
        if (CollectionTool.isNotEmpty(userIdList)  && siteId != null) {
            LOG.debug("向指定站点的指定总代后台{0}指定的在线登录用户ID{1}发送消息{2}", siteId, userIdList, message);
            sendMessageToUsers(siteId,userIdList,message);
        } else if (CollectionTool.isEmpty(userIdList) && siteId != null) {
            LOG.debug("向指定站点的所有总代后台{0}的所有在线登录用户发送消息{1}", siteId,message);
            ...
            LOG.debug("向所有总代后台的所有在线登录用户发送消息{0}", message);
