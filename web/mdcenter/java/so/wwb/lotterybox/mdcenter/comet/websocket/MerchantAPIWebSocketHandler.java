 ...
 */
public class MerchantAPIWebSocketHandler extends LbWebSocketHandler {
    private static final Log LOG = LogFactory.getLog(MerchantAPIWebSocketHandler.class);
    ...
    public void sendMessage(MessageVo message) {
        ...
        if (CollectionTool.isNotEmpty(userIdList)  && siteId != null) {
            LOG.debug("向指定站点API商户后台{0}指定的在线商户登录用户ID{1}发送消息{2}", siteId, userIdList, message);
            sendMessageToUsers(siteId,userIdList,message);
        } else if (CollectionTool.isEmpty(userIdList) && siteId != null) {
            LOG.debug("向指定站点API商户后台{0}的所有在线登录用户发送消息{1}", siteId,message);
            ...
            LOG.debug("向指定API商户后台{0}的所有在线登录用户发送消息{1}", siteIds,message);
            for (Integer itemSiteId : siteIds) {
                sendMessageToSite(itemSiteId,message);
            }
        }else{
            LOG.debug("向所有API商户后台的所有在线登录用户发送消息{0}", message);
