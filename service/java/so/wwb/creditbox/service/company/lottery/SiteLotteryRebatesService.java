...
//endregion your codes 1
    ...
    public SiteLotteryRebatesVo initRebatesData(SiteLotteryRebatesVo vo) {
        ...
        for (SiteLottery siteLottery : search) {
            ...
            switch (lotteryEnum){
                case CQSSC:
                    hashMap.put("1-5單碼",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"1_1",so.getParentHid()));
                    hashMap.put("1-5大小",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"2_11",so.getParentHid()));
                    hashMap.put("1-5單雙",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"3_13",so.getParentHid()));
                    hashMap.put("總和大小",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"16_71",so.getParentHid()));
                    hashMap.put("總和單雙",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"17_73",so.getParentHid()));
                    hashMap.put("龍虎",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"18_75",so.getParentHid()));
                    hashMap.put("和",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"18_77",so.getParentHid()));
                    hashMap.put("豹子",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"19_78",so.getParentHid()));
                    hashMap.put("順子",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"19_79",so.getParentHid()));
                    hashMap.put("對子",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"19_80",so.getParentHid()));
                    hashMap.put("半順",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"19_81",so.getParentHid()));
                    hashMap.put("雜六",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"19_82",so.getParentHid()));
                    break;
                case BJPK10:
                case XYFT:
                    hashMap.put("1-10單碼",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"1_1",so.getParentHid()));
                    hashMap.put("1-10大小",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"2_11",so.getParentHid()));
                    hashMap.put("1-10單雙",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"3_13",so.getParentHid()));
                    hashMap.put("1-5龍虎",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"4_15",so.getParentHid()));
                    hashMap.put("冠亞大",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"37_168",so.getParentHid()));
                    hashMap.put("冠亞小",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"37_169",so.getParentHid()));
                    hashMap.put("冠亞單",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"38_170",so.getParentHid()));
                    hashMap.put("冠亞雙",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"38_171",so.getParentHid()));
                    break;
                case GDKL10:
                    hashMap.put("1-8單碼",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"81_1",so.getParentHid()));
                    hashMap.put("1-8大小",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"82_21",so.getParentHid()));
                    hashMap.put("1-8單雙",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"83_23",so.getParentHid()));
                    hashMap.put("1-8方位",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"121_251",so.getParentHid()));
                    hashMap.put("1-8尾大小",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"84_27",so.getParentHid()));
                    hashMap.put("1-8合單雙",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"85_25",so.getParentHid()));
                    hashMap.put("1-8中發白-中發",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"122_283",so.getParentHid()));
                    hashMap.put("1-8中發白-白",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"122_285",so.getParentHid()));



                    hashMap.put("總和大小",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"11_245",so.getParentHid()));
                    hashMap.put("總和單雙",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"12_247",so.getParentHid()));
                    hashMap.put("總和尾大小",mapper.getRebateBetSortLine(lotteryEnum.getCode(),so.getSiteId(),so.getHid(),"13_249",so.getParentHid()));
